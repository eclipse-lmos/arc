// SPDX-FileCopyrightText: 2025 Deutsche Telekom AG and others
//
// SPDX-License-Identifier: Apache-2.0
package org.eclipse.lmos.arc.client.langchain4j.loaders

import org.eclipse.lmos.arc.agents.events.EventPublisher
import org.eclipse.lmos.arc.agents.llm.AIClientConfig
import org.eclipse.lmos.arc.agents.llm.ANY_MODEL
import org.eclipse.lmos.arc.agents.tracing.AgentTracer
import org.eclipse.lmos.arc.client.langchain4j.LangChainClient
import org.eclipse.lmos.arc.client.langchain4j.builders.azureOpenAiBuilder

class AzureOpenAiClientLoader : ClientLoader(
    name = "AZURE_OPENAI",
    dependOnClass = "dev.langchain4j.model.azure.AzureOpenAiChatModel",
    clientNames = setOf("langchain4j-azure", "langchain4j-azure-openai"),
) {

    override fun loadClient(
        config: AIClientConfig,
        tracer: AgentTracer?,
        eventPublisher: EventPublisher?,
    ) = buildMap {
        config.apiKey ?: error("API key is required for Azure OpenAI!")
        config.endpoint ?: error("Model endpoint is required for Azure OpenAI!")
        val client = LangChainClient(config, azureOpenAiBuilder(), eventPublisher, tracer)
        put(config.modelAlias ?: config.modelName ?: ANY_MODEL, client)
    }
}

